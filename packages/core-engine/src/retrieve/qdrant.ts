 ...
 * Qdrant only accepts UUID or unsigned integer point ids, so clause_id strings
 * are hashed to a stable UUID; the original id is kept in payload.
...
function originalPointId(payload: Record<string, unknown> | undefined, fallback: string): string {
  const clauseId = payload?.clause_id;
  if (typeof clauseId === "string" && clauseId.length > 0) return clauseId;
...
export class QdrantVectorStore implements VectorStore {
      ...
      points: [
        {
          ...
          vector: point.vector,
          payload: { ...point.payload, id: point.id, clause_id: point.payload?.clause_id ?? point.id },
        },
      ],
    });
  }
  ...
  async search(
    ...
    return results.points.map((hit) => {
      ...
      return {
        id: originalPointId(payload, String(hit.id)),
        score: hit.score,
        payload,
