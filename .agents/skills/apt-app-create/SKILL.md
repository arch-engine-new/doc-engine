---
name: apt-app-create
description: 从零做 App 产品 — PRD + 原型 + 页面逻辑 + 强制像素级规格载体（page.spec.*）
---

# $apt-app-create — 从零做 App 产品

你是产品产出代理。PM 给你一个产品想法，你产出**全部交付物**，并**强制**为每页（refine 时仅变更页）生成**像素级规格载体**（`page.spec.json` + `page.spec.png`）。
...
> 本命令是 Web `$apt-create` 的 App 分支：依赖原生 `targetPlatforms`；规格提取不可跳过（`--fast` 亦不免除）。  
...
- 空 / 仅 web / 无上述任一值 → **FAIL**，停止，并告知用户：
  ...
  > 当前项目无原生 targetPlatforms。请使用 Web 命令 `$apt-create`；或先在 `.apt/project.json` 配置原生平台后再跑 `$apt-app-create`。
...
`refine` 但磁盘无 PRD 或无 `_pages.md` → **FAIL**：提示改用 create，或先 `$apt-app-ingest`。
...
合法 `--fast` **不**免除原生门禁（§2）与强制 extract（§9）；若仍走 refine，**不**免除非空 `changePages`。

### 7. 产 PRD（gates 之后）

仅在 brief 已确认（或合法 `--fast`）且风格已确认（或合法 `--fast`）后，且 §6 自检通过：
...
### 7.1 正式 PRD 硬门禁（H8）— 产 PRD 后、产原型前
...
仅在 §7.1 H8 PASS 后，调 `generate_frontend_project`（契约 `ProductFlowAPI` / `GenerateFrontendInput`；复用 `pages` + `designTokens`）：
...
对须处理的页面跑 extract（`create`：`_pages.md` / 页面清单中的**每一页**；`refine`：**仅 `changePages` 变更页**）：

1. 确认存在可测 HTML：`designs/v0/<pageId>/index.html`（或等价可测量入口）
2. **解析 `extract-page-spec.cjs` 路径（按序，命中即用）——与 Web create 同序，禁止降级：**
...
3. 跑：`node <解析到的脚本> designs/v0/<pageId>/index.html`
...
**报告前再跑 H8：**再执行一次 `check-create-prd`（同 §7.1 路径解析）。FAIL → Overall **FAIL**（即使原型已生成），不得宣称 create 成功。

展示：
- `mode`（create / refine）与是否 `--fast`
- **正式 PRD 路径**（绝对或相对；缺合格正式 PRD → **FAIL**）
- 产出/变更了多少页
- 每页路径（含 `page.spec.json` / `page.spec.png`；refine 仅列变更页）
...
- **H5** `refine`：强制非空 `changePages`；`pages` ⊆ `changePages`；**禁止默认清空** `designs/v0`；**仅对 `changePages` 跑 extract**
...
- 无原生 targetPlatforms → **FAIL**（改用 `$apt-create`）；`--fast` 不免除
- 每须处理页强制 extract-page-spec（项目 `scripts/` → `$APT_HOME/scripts/` → FAIL 重装 APT）；失败或无 HTML → **FAIL** 列页
- **禁止**纯 Node HTML 解析 / puppeteer / 占位 PNG 冒充 `page.spec.*`；不得削弱 extract
